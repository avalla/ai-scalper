...
- `liquidation-cascade` BullMQ open/manage processor pair (the legacy
  in-process tick has shipped — see `apps/trader/src/trading/liquidation-cascade-tick.ts`).
  Backtest support (needs synthetic liquidation event stream).
...
| 10 | `liquidation-cascade` | `config.liquidation-cascade.json` | mean-reversion rebound after forced-liquidation cascades | ✅ **v1 live tick** | Requires `runtime.useWebSocket=true` AND the ws-feeder process running with `WS_LIQUIDATION_SYMBOLS` (or `WS_PUBLIC_TRADE_SYMBOLS`) covering the configured `liquidation.allowedSymbols`. Reader: `apps/trader/src/strategies/liquidations-cache-reader.ts` (Redis ZSET `ws:liquidations:<SYMBOL>`, populated cross-process by feeder). Tick: `apps/trader/src/trading/liquidation-cascade-tick.ts`. Single-leg, single-symbol, one-position-at-a-time. Exits: SL / TP / `liquidationMaxHoldSec` time-stop. Standard `evaluateRisk` (not aggressive-perps). BullMQ open/manage processor variant deferred. |
